...
func (c *CharacterLists) getAllCharacters(url string) {
	r, err := http.Get(url)
	if err != nil {
		return
	}
	...
	c.updateAllCharacters(data)
}

func (c *CharacterLists) updateAllCharacters(data Characters) {
	app.Dispatch(func() {
		c.characters = data
		c.Update()
	})
...
func (c *CharacterLists) isLoader() {
	app.Dispatch(func() {
...
func (c *CharacterLists) OnMount(ctx app.Context) {
	app.Dispatch(func() {
		c.getAllCharacters(c.Url)
	})
}

func (c *CharacterLists) onNext(ctx app.Context, e app.Event) {
...
func (c *CharacterLists) onPrev(ctx app.Context, e app.Event) {
	c.loader.loader = false
	c.Update()
...
func (c *CharacterLists) onPage(ctx app.Context, e app.Event) {
	e.PreventDefault()
	c.loader.loader = false
	c.Update()
...
func (c *CharacterLists) Render() app.UI {
